...
`scripts/stacker-scene-mqtt-sequence.mjs` 会按 test2 现场流程发送完整动画报文：货箱 `Box01` 先出现在链条机 `1005` 前端，脚本发送前端有货信号、`payload=Box01` 绑定和 `movement_x=1` 输送；到后端后发送后端有货和停止信号，随后 `DDJ2` 移动到 1005 后端、伸叉取货、缩回到载货台，再移动到定位线框 `1-1-1`，最后通过 `cargo_action=drop`、`cargo=Box01`、`target=1-1-1` 放货。脚本默认设备号为 `DDJ2` 和 `1005`；加 `--plc` 或设置 `STACKER_SCENE_PROTOCOL=plc` 时仅把 DDJ2 joint 帧转换为 PLC 点位和 `{data,ts}` 包装，链条机和货箱帧保持标准 `{e,p,v}` 语义。本地 WebSocket 默认地址为 `ws://127.0.0.1:18084/stacker-scene`。
...
1. 场景中准备链条机和 Stacker，资产编号分别填写 `1005`、`DDJ2`。
2. 创建/拖入一个 cube 作为货箱，资产编号填写 `Box01`。
3. 创建定位线框立方体，资产编号填写 `1-1-1`；test2 当前场景已补齐该定位线框。
...
默认配置可通过环境变量覆盖：`STACKER_SCENE_MQTT_HOST`、`STACKER_SCENE_MQTT_PORT`、`STACKER_SCENE_WS_HOST`、`STACKER_SCENE_WS_PORT`、`STACKER_SCENE_WS_PATH`、`STACKER_SCENE_PROTOCOL`、`STACKER_SCENE_PAYLOAD_WRAP`、`STACKER_SCENE_STACKER_ID`、`STACKER_SCENE_DEVICE_CODE`、`STACKER_SCENE_CONVEYOR_ID`、`STACKER_SCENE_BOX_ID`、`STACKER_SCENE_LOCATOR_ID`、`STACKER_SCENE_BOX_FRONT`、`STACKER_SCENE_BOX_REAR`、`STACKER_SCENE_PICKUP_DISTANCE_X`、`STACKER_SCENE_DROP_DISTANCE_X`、`STACKER_SCENE_PICKUP_DISTANCE_Y`、`STACKER_SCENE_DROP_DISTANCE_Y`、`STACKER_SCENE_FORK_EXTEND`、`STACKER_SCENE_FORK_RETRACT`、`STACKER_SCENE_TIME_SCALE`。其中位置变量支持 `x,y,z` 逗号格式或 JSON 对象。若只想向 MQTT broker 自动播放而不等待编辑器 WebSocket 连接，可执行 `npm run demo:stacker-scene -- --autostart`；若只走本地 WebSocket 可执行 `npm run demo:stacker-scene -- --no-mqtt`。
...
- 2026-06-22：完成 test2 `1005 -> DDJ2 -> 1-1-1` 动画流程脚本和场景定位线框补齐；已执行 `npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、test2 场景 JSON 资产编号校验和 `npm run build`，构建通过，Vite 仅保留既有大 chunk 警告。
