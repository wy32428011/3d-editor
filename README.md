...
- 工具栏：选择、移动、旋转、缩放、创建基础对象、创建定位线框立方体、撤销场景布局、导入、导入 CAD 图纸、保存、发布、统一数据源配置、整场景动画预览、正顶俯瞰模式、Inspector、画质选择和 GPU 诊断
...
- 多档画质选择：工具栏提供“无损、自动、均衡、流畅”四档；“自动”会先按无损高清渲染，只有复杂场景低帧率时才临时提高 `hardwareScalingLevel`、关闭网格光晕后处理并降低编辑辅助动画开销；“均衡”约按 1440p 目标像素渲染，“流畅”则主动降低分辨率并关闭指针移动拾取，便于大场景快速预览。
...
- FPS 流畅度优化：大场景会缓存结构性统计、POI 节点和数据驱动目标，避免状态栏、业务运行态和模拟数据源反复全场景扫描；当 mesh/顶点/active mesh/draw call 规模过大或 FPS 偏低时，会自动关闭编辑网格光晕后处理和闪烁动画，并暂停指针移动拾取，把帧预算优先留给场景模型本体；点击、右键和拖放仍使用显式拾取。
...
- 2026-06-13：工作网格升级为同步呼吸闪烁和专用 GlowLayer 光晕；光晕只 include 网格闪光主线、坐标轴和定位线，仍保持 helper 不参与拾取、层级树和场景序列化，降负载画质会关闭光晕后处理并保留基础线段显示。
- 2026-06-21：左侧模型树拖拽归组支持当前多选集合；拖拽已选节点时会把选区内未锁定的顶层模型或 group 一起移入目标 group，拖到树空白根区域时会一起移回根级，并继续拦截拖入自身、后代或锁定 group 的非法层级。
- 2026-06-21：补齐大场景流畅度优化第一阶段；Electron 启动增加 `enable-zero-copy`、Windows `use-angle=d3d11` 和窗口 `backgroundThrottling=false`，开发态 DevTools 改为 `ELECTRON_OPEN_DEVTOOLS=1` 显式开启；Babylon 新增“无损、自动、均衡、流畅”四档画质，默认无损高清不自动降画质，只有自动档会在低 FPS 时临时提高 `hardwareScalingLevel`；运行态新增场景统计、POI 节点、可绑定业务根节点和数据驱动目标缓存，大场景或低 FPS 时自动关闭网格光晕、闪烁动画和指针移动拾取，减少 CPU 全量扫描、鼠标移动拾取与 GPU 后处理开销；状态栏新增 WebGL 版本、最大纹理尺寸、软件渲染识别和自动降压提示。
