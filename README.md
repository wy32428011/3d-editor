...
辊道机和链条机现场 PLC 点位按 `{data:[{e,p,v}],ts}` 合帧后直接消费：新协议以 `signalBits` 表示光电信号，bit0 为前端有货、bit3 为后端有货，bit0 上升沿会在预览态生成临时 cube 货箱；旧 `move` bit0 和 `front_has_cargo=true` 仍兼容。`movement_x` 是输送方向，`0` 不转、`1` 链条/滚筒正转、`2` 链条/滚筒反转；`movement_y` 保留给顶升类垂直动作，`rotation` 保留给旋转台方向。新 `action` 是抱机业务动作，`0` 没任务、`1` 抱束中、`2` 抱束完成、`3` 解抱束中、`4` 解抱束完成，不再作为输送方向；旧 PLC 位域 `action` 只会在同帧没有显式 `movement_x` 时兼容映射。货箱编号优先取 `containerCode/container_no`，缺失时用非 0 `task` 生成 `Task202` 这类稳定编号；临时 cube 不写入场景文件，停止预览时清理。
...
`scripts/stacker-scene-mqtt-sequence.mjs` 会按 test2 现场流程发送完整动画报文：脚本读取 `C:\Users\WY\Documents\test2\test2` 当前主场景，确认资产编号 `1004` 的链条机、`DDJ2` 和定位线框 `1-1-1` 存在，并用 `1004` 的 `chainLength`、`chainFrontEndpointRatio`、`chainRearEndpointRatio` 估算输送等待时间。流程开始时在 `1004` 的 `twindatadriven/joint` 发送新协议 `{data:[{e,p,v}],ts}` 完整点位包，字段包含 `deviceCode/mode/action/task/movement_x/movement_y/signalBits/containerCode/workingHours_x/workingHours_y/normal/errorCode/message/layer/rotation/container_quantity/folding/flip/fork/result/result2`；默认 `task=202`、`containerCode=""`，因此运行态货箱编号为 `Task202`，`signalBits bit0` 生成预览态 cube，随后周期发送 `movement_x=1` 保活帧，避免运行时 5 秒断流保护让货箱停在半程，直到货箱沿链条机 `chainFrontEndpointRatio -> chainRearEndpointRatio` 端点轨迹走到后端后再发送 `movement_x=0` 和 `signalBits bit3` 表示后端有货；流程不发布 `material/{boxId}/twinspawn` 货箱位姿帧。到后端后，`DDJ2` 通过 `travel_target=1004,target_anchor=rear` 按链条机模型后端取货位移动，使用 `fork_target=Task202` 按运行态货箱模型伸叉取货，再通过 `travel_target=1-1-1` 移动到定位线框，并用 `fork_target=1-1-1`、`cargo_action=drop`、`drop_target=1-1-1` 放货。该流程不发送 `distance_x/distance_y/front_distance_z/back_distance_z` 这类距离定位字段；已有 PLC 距离字段仍由运行时兼容，同帧存在模型目标字段时以模型目标定位为准。脚本默认设备号为 `DDJ2` 和 `1004`；加 `--plc` 或设置 `STACKER_SCENE_PROTOCOL=plc` 时仅把 DDJ2 动作点位转换为 PLC 位域，目标字段仍保留为 MQTT 点位。本地 WebSocket 默认地址为 `ws://127.0.0.1:18084/stacker-scene`。
