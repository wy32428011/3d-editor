...
- 编辑视口相机：编辑态和预览态的鼠标导航都采用 Web 3D 查看器风格；左键拖拽直接围绕当前 pivot 环绕视角，左键单击仍用于编辑态选择对象，命中 Gizmo 时保留移动、旋转和缩放轴拖拽；右键或中键拖拽沿屏幕平面平移视角，`Alt + 右键` 拖拽缩放，滚轮沿当前视线前进/后退并允许贴近、穿过模型内部；按住 `Shift` 会加快环绕、平移、缩放和滚轮速度；右键单击命中可编辑对象时只在编辑态打开对象菜单，预览态不会弹出编辑菜单；工具栏可切换正顶俯瞰模式，开启时先切到近似垂直正顶视角，之后主动旋转只改变水平朝向且不会切换俯仰角，滚轮、中键/右键平移、`Alt + 右键` 缩放和显式聚焦仍可使用，该模式下 Gizmo 拖拽和属性面板变换不会自动带动相机视角
- 默认场景：编辑网格、场景内 X/Y/Z 坐标轴、环境光、方向光、立方体、球体、地面；项目条和右侧场景属性可配置场景环境背景色，避免视口背景固定为全黑
- 米制单位：编辑器约定 `1 Babylon unit = 1 m`，默认对象、工作网格、属性面板位置和模型尺寸都按 `m` 展示；导入模型不会修改源文件，编辑器会在 Babylon/glTF 节点变换生效后按真实世界包围盒阈值启发式推断米、厘米或毫米来源，并把进入场景的模型实例归一到米，单位决策会写入模型和资产元数据；工作网格默认每小格为 `1 m`，会随相机位置和缩放动态覆盖视口，并通过全网格同相位呼吸闪烁和专用光晕层帮助定位编辑平面；工具栏可显式显示/隐藏工作网格，也可单独开启/关闭网格呼吸效果，画质或降载模式不会改写这两个用户开关；光晕层只作用于编辑器网格 helper，不影响模型拾取、CAD、选中描边或场景保存
- 工具栏：选择、移动、旋转、缩放、创建基础对象、创建定位线框立方体、撤销场景布局、导入、导入 CAD 图纸、保存、发布、统一数据源配置、整场景动画预览、正顶俯瞰模式、工作网格显示开关、网格呼吸效果开关、Inspector、画质选择和 GPU 诊断
- GizmoManager：选中可变换对象后会自动切换到移动工具并显示 X/Y/Z 方向轴，仍支持手动切换旋转和缩放；锁定对象和预览模式下不会显示移动轴
- OutlineRenderer：选中对象只显示外轮廓，不覆盖模型主体材质和贴图
- 点击拾取：从视口或层级树选中对象，普通单击只更新选区、Gizmo 和属性面板，不改变当前相机视角；双击层级节点或视口模型可快速定位到对象；左侧模型树支持 Windows 风格多选，普通点击替换选区，`Ctrl` 点击切换单项，`Shift` 按当前可见行做范围选择，选区会同步到树行高亮和视口轮廓高亮；左侧模型树支持新建逻辑 group、单模型或多选模型拖拽归组、拖回根级、group 展开折叠、显隐切换和锁定保护，选中 group 时会高亮其下所有普通 mesh 模型
...
- RGV 参数形态：`models/RGV/rgv.model.ts` 暴露 `vehicleLength`、`vehicleWidth` 和 `vehicleHeight` 三个米制动态参数，右侧属性面板会显示为 RGV 长度、宽度和高度；运行脚本以 `vehicleLength`、`vehicleWidth`、`vehicleHeight` 的默认米制值作为稳定基线，按模型视觉语义把长度映射到 Z 轴、宽度映射到 X 轴、高度映射到 Y 轴做根节点参数化缩放，避免每次参数刷新时用当前包围盒反向抵消用户输入。
...
- 链条机参数形态：`ChainConveyor01` / `chain-conveyor` 模型包不再用根节点 Z 向缩放表达 `chainLength`，也不再用根节点 X 向缩放表达 `chainWidth`；编辑器会记录 `Rail_01_M001`、`Rail_02_M001`、`ZJ`、`ZJ01`、`Box003`、`Box004` 和 `DJ` 的原始基线，长度变化时固定局部 `-Z` 起点，将两侧导轨和长梁类节点的 Z 向顶点延展到局部 `+Z` 方向，`Box004` 尾端附件跟随新尾端平移。`DJ` 电机保持图 1 的中间驱动位置，不参与缩放、不随长度尾端移动；`Box003` 会先把 GLB 因法线/材质拆出的同坐标顶点焊接成物理连通块，再以 `DJ` 的 X/Z 中心保护中间驱动段，长度增量只从驱动段尾侧之后作用到真正的长梁组件，横梁、脚板和支撑脚按组件保持原始三维形状，避免中间支撑板和电机座被拉长、拉斜或跑到端部。宽度变化时以局部 X 中心为锚点，`Box003` 会按物理连通块区分侧边窄块和内部跨宽块：两侧长梁/支撑小块保持原截面整体外移或内移，跨宽横梁、脚板、支撑脚等内部主体按左右顶点延展到新的宽度，只有 `DJ` 驱动区内的紧凑小件保持原始截面与位置；`Rail_01_M001`、`Rail_02_M001`、`ZJ`、`ZJ01` 和 `Box004` 作为刚体跟随两侧边界移动，导轨、支架、电机和侧边附件不会被横向压扁或放大截面，也不会和内部主体脱开架空。若 `Rail_01_M001` / `Rail_02_M001` 导出为空 `TransformNode` 包装，编辑器会记录并延展其真实子 Mesh 顶点；红框外主体会随 `chainLength` 真实变长，调小时也会从基线重新计算到不反转网格的安全长度，但仍不使用根节点整体缩放；保存重开、复制粘贴和阵列后都会按实例独立基线重新应用当前长度和宽度。链条机 fallback 识别以 `Box003`、`Rail_01_M001`、`Rail_02_M001` 和 `DJ` 为核心节点，`ZJ/ZJ01/Box004` 等附件缺失时不会导致长度和宽度参数完全失效。新增 `chainFrontEndpointRatio` 和 `chainRearEndpointRatio` 两个端点参数，默认前端为 `1`、后端为 `0`，沿模型局部 `-Z` 到 `+Z` 的当前输送段比例定位；MQTT 运行时的 `target_anchor=front/rear` 与 test2 场景脚本的货箱前后端都会优先使用该参数，旧场景缺失时按默认值兜底。
...
- 默认无损高清渲染：视口默认使用“无损”画质，按 3840x2160 目标像素量设置 Babylon 后备缓冲，低于 4K 的窗口自动超采样，避免高 DPI/大屏下画面发糊或出现马赛克；该模式不会因为低 FPS 自动降画质。
- 多档画质选择：工具栏提供“无损、自动、均衡、流畅”四档；“自动”会先按无损高清渲染，只有复杂场景低帧率时才临时提高 `hardwareScalingLevel` 并减少指针移动拾取开销；“均衡”约按 1440p 目标像素渲染，“流畅”则主动降低分辨率并关闭指针移动拾取，便于大场景快速预览。画质选择不会自动关闭、削弱或开启网格呼吸效果。
- GPU 渲染诊断：工具栏状态栏可悬停查看真实渲染分辨率、硬件缩放、WebGL 版本、纹理上限和 GPU renderer，若落到 SwiftShader / Microsoft Basic Render Driver 等软件渲染路径会直接显示“软件渲染”。
- FPS 流畅度优化：大场景会缓存结构性统计、POI 节点和数据驱动目标，避免状态栏、业务运行态和模拟数据源反复全场景扫描；当 mesh/顶点/active mesh/draw call 规模过大或 FPS 偏低时，会暂停指针移动拾取，把帧预算优先留给场景模型本体；网格显示和呼吸光晕只按工具栏开关执行，不由自动降载策略接管；点击、右键和拖放仍使用显式拾取。
...
开发模式默认不再自动打开 DevTools，避免调试器影响 GPU/帧率观测；需要调试时可显式设置：

```bash
set ELECTRON_OPEN_DEVTOOLS=1&& npm run electron:dev
```
...
模型包脚本可以导出 `dataDriven` 对象，把运动语义放回模型资产自身。`motion.*.valueMode` 缺省为 `"target"` 以兼容旧场景，表示 payload 的 `v` 是相对进入预览基线姿态的目标值；新模型包使用 `valueMode:"action"`，表示 payload 的 `v` 是动作枚举，运行时按 `direction * speed * deltaSeconds` 持续积分。`translate` 的速度单位为 `m/s`，`rotate` 的速度单位为 `deg/s`；动作值为 `0` 时停止并保持当前位置，真实连接超过 5 秒无新消息时也会自动停住。`actionMap` 用来把协议枚举映射成方向：通用 `0=0, 1=+1, 2=-1`；货叉 `1/3=+1` 表示伸出，`2/4=-1` 表示缩回；输送线/辊筒 `1=+1, 2=-1` 表示正反转。`target:"root"` 可让 RGV/AGV/四向车这类整车按本地轴持续移动，缺省 `target:"nodes"` 驱动内部节点。`motion.*.limits` 仍可声明 `min/max` 或防撞物体推导边界，动作积分和旧 target 目标都会被截断在安全范围内；Stacker 若没有端部防撞节点，会使用上下轨道几何范围兜底生成行走边界。Stacker 的 `distancex/distance_x` 使用协议原值校准行走位置，轨道起点作为 0 点，并优先于同帧 `action/movement_x` 积分；新协议 `distance_y` 使用米制浮点校准载货台高度，旧 `front_distanceY/back_distanceY` 仍按 PLC 毫米值换算成米后校准升降，并优先于同帧 `front_action/back_action/movement_y` 积分；Stacker 模型包参数 `forkLength`（兼容旧键 `fork_length`、中文“货叉长度”和“货叉总长度”）表示货叉动画可伸出的总长度，不直接缩放编辑态静态货叉模型，运行态按取货方向生成左右正负行程，按取货深浅使用完整或半段行程；若实例没有该参数，会回退到 `motion.fork.limits` 或默认 `0.941m`；新协议 `front_distance_z/back_distance_z`、V5.2 标准 `front_forkLocation/back_forkLocation` 和扩展 `front_forkDistance/back_forkDistance` 都可作为货叉绝对伸缩位置校准，并会把伸缩目标夹紧在 `[-forkLength,+forkLength]`；命中这些校准字段时优先于同帧 `front_movement_z/back_movement_z/forkAction` 积分；`rpm_*`、`workingHours_*`、`signalBits` 等字段作为协议遥测保留，不直接换算位移或转速；整机绝对位姿仍使用 `twinspawn` 的 `x/y/h/r`。

Stacker 按 MQTT 文档中的数字孪生主题接入：整机生成/位姿使用 `dt/factory/logistics/stacker/DDJ2/twinspawn`，内部机构动作使用 `dt/factory/logistics/stacker/DDJ2/twindatadriven/joint`；运行时会同时把现场 `DDJ2` 和旧模型包默认 `Stacker01` 作为 Stacker 设备号兜底匹配，手动填写右侧“资产编号”仍会优先参与匹配。文档坐标中 `x/y/h/r` 分别表示 X 坐标、Y 坐标、Z 高度和旋转角度；运行时会映射到 Babylon 场景的 `x/z/y/rotationY`，也就是文档平面 Y 落到编辑器地面 Z 轴，文档高度 H 落到编辑器垂直 Y 轴。`twindatadriven/joint` 中的 `{e,p,v}` 会在运行时归一为 `{e,[p]:v}` 后再驱动模型内部运动。
...
动作枚举含义：`movement_x: 1` 前进、`2` 后退、`0` 静止；`movement_y: 1` 上升、`2` 下降；货叉 `front_movement_z/back_movement_z/forkState: 1/3` 伸出、`2/4` 缩回。`e` 和 `deviceCode` 都可匹配设备；规范 MQTT topic 也会提供 `{deviceId}` 兜底。`cargo_action=pickup/drop` 配合 `cargo=货箱资产编号` 使用，运行态会在货叉伸出量和距离满足保护阈值时把货箱临时吸附到货叉吸附点；`drop` 不带 `target` 时解除吸附并保持货箱当前位置，带 `target=定位框资产编号` 时会把已吸附货物底面中心放入对应定位线框底面中心，定位框只需在“资产信息”中填写资产编号，不必启用动画连接。规范 `twindatadriven/payload` 的 `{p:"payload",v:"Box01"}` 会归一为货箱字段，用于同步载体与负载关系：发给 Stacker 时同步货叉载荷，发给有运动语义的输送线时可绑定货箱到该输送线并由 `movement_x` 启停推进货箱；当前 opaque 辊道机 `RollerConveyor01` 和链条机 `ChainConveyor01` 都支持前端有货上升沿生成预览态临时 cube，货箱编号优先取 `containerCode/container_no`，缺失时取非 0 `task`，再缺失才生成运行态编号。`RollerConveyor01` 的 `movement_x/rotation` 会同时驱动原始 `GT` 辊筒和参数化生成的同源辊筒逐根自转，运行时会把自转 pivot 校正到每根辊筒自身几何中心，并推动货箱沿辊道有界移动；`ChainConveyor01` 收到 `movement_x` 时不再驱动 `Rail_01_M001/Rail_02_M001` 整体转动，Rail 保持主体装配不动，运行态会在输送面显示循环移动的光晕箭头指明方向，并按 `chainFrontEndpointRatio` 到 `chainRearEndpointRatio` 的模型端点语义推进货箱。若数据服务外层包了一层 `data` 或 `payload`，可以把“数据路径”设置为对应点路径；运行时也会递归归一化包装内的 `{e,p,v}` joint 数组，并会把外层 `target/dropTarget/locator` 放货目标透传到内层业务帧。数组 payload 会按设备合并处理但单条消息最多消费 200 帧，避免高频大包造成内存压力。

辊道机和链条机现场 PLC 点位按 `{data:[{e,p,v}],ts}` 合帧后直接消费：新协议以 `signalBits` Integer 枚举表示光电信号，`0` 前端有货、`1` 前端换速、`2` 后端换速、`3` 后端有货、`4` 低位停准（顶升设备）、`5` 高位停准（顶升设备）；运行态仅在 `signalBits=0` 且货箱数量未明确为 0 时生成临时 cube 货箱，`signalBits=3` 表示后端有货，`1/2/4/5` 作为状态保留。旧 `move` 来货信号和 `front_has_cargo=true` 仍兼容。`movement_x` 是输送方向，`0` 不转、`1` 链条/滚筒正转、`2` 链条/滚筒反转；`movement_y` 保留给顶升类垂直动作，`rotation` 保留给旋转台方向。新 `action` 是抱机业务动作，`0` 没任务、`1` 抱束中、`2` 抱束完成、`3` 解抱束中、`4` 解抱束完成，不再作为输送方向；链条机目标不会把由旧 `action` 位域推导出的 `movement_x` 当作输送动作，真实上报的 `movement_x` 仍是唯一方向来源。货箱编号优先取 `containerCode/container_no`，缺失时用非 0 `task` 生成 `Task202` 这类稳定编号；临时 cube 不写入场景文件，停止预览时清理。

现场 DDJ2 Stacker PLC 报文可直接使用当前 `{data:[{e,p,v}],ts}` 合帧格式，`e` 作为模型匹配号，`deviceCode` 作为调度号保留，不覆盖 `e`；运行时仍兼容旧报文字段 `device_code`：
...
DDJ2 PLC 点位映射按当前截图协议处理：`movement_x` 直接驱动行走，`1` 前进、`2` 后退、`0` 停止；`distance_x` 是 Stacker 行走绝对距离，运行时按协议原值以轨道起点为 0 点校准行走位置，同帧存在 `movement_x` 时以 `distance_x` 为准；`movement_y` 直接驱动载货台升降，`1` 上升、`2` 下降、`0` 停止；`distance_y` 是米制载货台高度校准值，优先于同帧升降动作；`front_movement_z/back_movement_z` 直接驱动货叉伸缩，`1/3` 伸出、`2/4` 缩回、`0` 停止；`front_distance_z/back_distance_z` 是货叉伸缩距离校准值，最终夹紧在当前 Stacker 实例 `[-forkLength,+forkLength]` 范围内，并优先于同帧货叉动作。运行时继续兼容旧 V5.2 写法：`action` bit0/bit1 可转 `movement_x`，`front_action/back_action` bit2/bit3 可转 `movement_y`，`front_forkAction/back_forkAction` 可转货叉动作，`distancex`、`front_distanceY/back_distanceY`、`front_forkLocation/back_forkLocation` 仍可作为旧协议校准字段。`front_x/front_y/front_z`、`to_x/to_y/to_z`、`front_command/back_command`、`front_task/back_task`、`front_containerCode/back_containerCode`、`signalBits/front_signalBits/back_signalBits`、`rpm_*`、`workingHours_*`、`normal/errorCode/message` 作为业务状态或遥测字段保留，默认不直接换算模型位移和速度；输送线的 `signalBits=0/3` 光电枚举只在辊道机/链条机目标上解释。
...
Stacker 的行走会在进入运动缓存前应用防越界限制。模型包显式声明 `motion.*.limits.min/max` 时按该数值截断；未声明数值但提供两端 `blockerNodes` 时，运行时会读取防撞物体几何包围盒的内侧面，保证行走机构整体不会越过两端防撞物体；如果模型只有上下轨道 `fixedNodes`，运行时会改用轨道整体几何范围兜底生成边界。`distance_x/distancex`、`movement_x` 动作和 `twinspawn` 平面位置都会被夹紧在进入预览时的轨道范围内，高度 `h` 与朝向 `r` 保持原协议语义；`distance_y` 和旧 `front_distanceY/back_distanceY` 会按 lift 限位截断；Stacker 货叉运行态支持左右双向伸缩，当前实例的 `forkLength` 是动画最大伸出总长度并优先于模型包旧的固定 `motion.fork.limits`，运行时会生成 `-forkLength..+forkLength` 的货叉行程；该参数不会直接拉伸静态模型货叉，缺少实例参数时才回退到 `motion.fork.limits` 或默认 `0.941m`。超范围距离或动作只会在边界停住，不会报错或断开预览。Stacker 模型脚本已声明物理速度：`travel.speed=0.8m/s`、`lift.speed=0.3m/s`、`fork.speed=0.25m/s`。

除 Stacker 外，`E:\公司文件\数字孪生\模型文件\models` 下已补充以下物理动作语义：多穿小车 `Shuttle01` 使用 `front_movement_z/back_movement_z/forkState` 驱动货叉伸缩；RGV `RGV01` 使用 `movement_x/movement_y` 按根节点本地轴持续移动；`RollerConveyor01` 辊道机、有电机辊道 `MotorConveyor01` 和弯道输送机 `WLTS01` 使用 `movement_x` 或 `rotation` 驱动辊筒正反转；链条机 `ChainConveyor01` 使用 `movement_x` 推进运行态货箱沿 `chainFrontEndpointRatio -> chainRearEndpointRatio` 端点轨迹输送，Rail 静态装配件不再整体转动，输送面用移动光晕箭头表达正反向；一体式顶升移载 `YZJ01` 使用 `movement_y` 顶升、`movement_x/rotation` 驱动移载辊；换层提升机 `HCTS01` 使用 `movement_y` 驱动轿厢/平台升降。`RollerConveyor01` 还支持 `payload` 绑定货箱并做辊筒范围内的有界输送，现场 PLC `move` bit0 前端有货时会自动生成预览态临时 cube。Shelf 是静态货架，LED 属于状态灯颜色语义，本轮不加入物理动作。
...
桥接演示步骤：先启动脚本，再拖入 `E:\公司文件\数字孪生\模型文件\models\Stacker` 和资产编号为 `Box01` 的货箱，选中 Stacker，在右侧“数据驱动”中点击“填入 Stacker Demo”，然后进入预览模式。按钮会把绑定设备写为 `DDJ2`，并把场景级数据源设置为本地 WebSocket demo；脚本默认持续发送 DDJ2 新现场完整点位包，直接使用 `movement_x/movement_y/front_movement_z/back_movement_z` 和 `distance_x/distance_y/front_distance_z/back_distance_z`，便于确认上下轨道固定、行走机构沿轨道移动、载货台升降、货叉伸缩和货箱跟随。

### DDJ2 新现场报文 Demo

`scripts/stacker-ddj2-plc-message.mjs` 使用截图协议里的完整新报文字段集合，每帧都输出 `{data:[{e:"DDJ2",p:"...",v:"..."}],ts:"..."}`。脚本只覆盖少量动作和编码点位形成演示动作，其他 `mode/command/task/containerCode/signalBits/rpm/workingHours/normal/errorCode/message/to_*` 字段保持现场报文形态，用于 dry-run 和现场联调核对；默认 `deviceCode=1`，可通过 `STACKER_DDJ2_DEVICE_CODE` 覆盖。
...
`scripts/logistics-mqtt-transformer.mjs` 用于把现场 PLC 原始报文转换成编辑器规范 Topic，适合一个场景同时存在 Stacker、链条机、RGV 等多台设备时统一接入。配置文件为 `scripts/logistics-mqtt-transformer.config.json`，按 `profiles / devices / maps` 分层：`profiles` 描述一类设备怎么转换，`devices` 描述场景中的设备实例，`maps` 保存货位和资产映射；同类设备新增实例时通常只需要在 `devices` 里追加设备编号和资产编号。

```bash
npm run demo:logistics-transformer:dry-run
```

默认 dry-run 样例会把 DDJ2 的 `front_command=5`、`front_containerCode=BOX001`、`to_z/to_x/to_y=1/9/3` 转换成 `cargo_action=drop`、`cargo=BOX001`、`target=1-9-3`，并保留 `movement_x/distance_x/movement_y/distance_y/front_movement_z/front_distance_z` 等动画字段。启动真实转换器时执行：

```bash
npm run demo:logistics-transformer
```

默认配置把源 MQTT 视为现场原始区 `dt/factory/plc/raw/+/+`，输出到编辑器规范 Topic `dt/factory/logistics/{devType}/{deviceId}/twindatadriven/joint`；如果现场源 Topic 已经固定在规范 Topic 上，建议让转换器输出到单独 broker 或本地 WebSocket，避免订阅和发布同一个 Topic 造成循环消费。本地 WebSocket mirror 默认地址为 `ws://127.0.0.1:18086/logistics-transformer`，可用于编辑器 WebSocket 数据源联调。

### Stacker 场景流程 MQTT 报文 Demo

`scripts/stacker-scene-mqtt-sequence.mjs` 会按 test2 现场流程发送完整动画报文：脚本读取 `C:\Users\WY\Documents\test2\test2` 当前主场景，确认资产编号 `1004` 的链条机、`DDJ2` 和定位线框 `1-1-1` 存在，并用 `1004` 的 `chainLength`、`chainFrontEndpointRatio`、`chainRearEndpointRatio` 估算输送等待时间。流程开始时在 `1004` 的 `twindatadriven/joint` 发送新协议 `{data:[{e,p,v}],ts}` 完整点位包，字段包含 `deviceCode/mode/task/movement_x/movement_y/signalBits/containerCode/workingHours_x/workingHours_y/normal/errorCode/message/layer/rotation/container_quantity/folding/flip/fork/result/result2`，`ts` 使用东八区 `+08:00` 格式；默认 `task=202`、`containerCode=""`，因此运行态货箱编号为 `Task202`，`signalBits=0` 生成预览态 cube，随后周期发送 `movement_x=1` 保活帧，避免运行时 5 秒断流保护让货箱停在半程，直到货箱沿链条机 `chainFrontEndpointRatio -> chainRearEndpointRatio` 端点轨迹走到后端后再发送 `movement_x=0` 和 `signalBits=3` 表示后端有货；流程不发布 `material/{boxId}/twinspawn` 货箱位姿帧。到后端后，`DDJ2` 通过 `travel_target=1004,target_anchor=rear` 按链条机模型后端取货位移动，使用 `fork_target=Task202` 按运行态货箱模型伸叉取货，再通过 `travel_target=1-1-1` 移动到定位线框，并用 `fork_target=1-1-1`、`cargo_action=drop`、`drop_target=1-1-1` 放货。该流程不发送 `distance_x/distance_y/front_distance_z/back_distance_z` 这类距离定位字段；已有 PLC 距离字段仍由运行时兼容，同帧存在模型目标字段时以模型目标定位为准。脚本默认设备号为 `DDJ2` 和 `1004`；加 `--plc` 或设置 `STACKER_SCENE_PROTOCOL=plc` 时，DDJ2 动作帧会包装成现场完整点位包，不再转换旧 `action/front_action/front_forkAction` 位域，`travel_target/fork_target/drop_target` 等动画目标字段会作为扩展点位附加在同一 `data` 数组末尾。本地 WebSocket 默认地址为 `ws://127.0.0.1:18084/stacker-scene`。
...
1. 场景中准备链条机和 Stacker，资产编号分别填写 `1004`、`DDJ2`。
2. 创建定位线框立方体，资产编号填写 `1-1-1`；test2 当前场景已补齐该定位线框。
3. 不需要预置货箱模型；`1004` 前端有货信号会在预览态自动生成 cube，停止预览后清理。
...
默认配置可通过环境变量覆盖：`STACKER_SCENE_PROJECT_PATH`、`STACKER_SCENE_FILE_PATH`、`STACKER_SCENE_MQTT_HOST`、`STACKER_SCENE_MQTT_PORT`、`STACKER_SCENE_WS_HOST`、`STACKER_SCENE_WS_PORT`、`STACKER_SCENE_WS_PATH`、`STACKER_SCENE_PROTOCOL`、`STACKER_SCENE_PAYLOAD_WRAP`、`STACKER_SCENE_STACKER_ID`、`STACKER_SCENE_DEVICE_CODE`、`STACKER_SCENE_CONVEYOR_ID`、`STACKER_SCENE_CONVEYOR_TASK`、`STACKER_SCENE_CONVEYOR_CONTAINER_CODE`、`STACKER_SCENE_BOX_ID`、`STACKER_SCENE_LOCATOR_ID`、`STACKER_SCENE_CONVEYOR_CARGO_SPEED`、`STACKER_SCENE_CONVEYOR_PADDING_MS`、`STACKER_SCENE_TIME_SCALE`。若只想向 MQTT broker 自动播放而不等待编辑器 WebSocket 连接，可执行 `npm run demo:stacker-scene -- --autostart`；若只走本地 WebSocket 可执行 `npm run demo:stacker-scene -- --no-mqtt`。

### test2 真实 MQTT 现场配置

`C:\Users\WY\Documents\test2\test2` 当前主场景已写入真实 MQTT over WebSocket 配置：数据源类型为 `MQTT`，连接地址为 `ws://192.168.60.154:8083/mqtt`，订阅 Topic 为 `dt/factory/logistics/+/+/twinspawn` 和 `dt/factory/logistics/+/+/twindatadriven/joint`，设备字段为 `e`，匹配字段为 `assetCode`，插值时间为 `200ms`。打开 test2 项目并进入预览后，运行态会直接连接该 broker；若 `twinspawn` 或 `joint` payload 缺少 `e`，规范 topic 中的设备号仍会作为兜底匹配场景资产编号。

2026-06-24 已用 `ClientWebSocket` 验证 `192.168.60.154:8083` 可握手到 `/mqtt` 且子协议为 `mqtt`，MQTT `CONNACK=0`，订阅 `dt/factory/logistics/+/+/twinspawn` 和 `dt/factory/logistics/+/+/twindatadriven/joint` 都收到 `SUBACK`。`joint` topic 已收到现场 conveyor 设备的 `{data:[{e,p,v}],ts}` 完整点位包，结构与运行时兼容；当次 15 秒指定订阅 `conveyor/1004` 和 `stacker/DDJ2` 未收到 `PUBLISH`，因此若当前 test2 场景模型不动，应先确认现场是否正在向 `1004` 或 `DDJ2` 对应 Topic 持续发布数据。
...
- 2026-06-26：新增多设备 Logistics MQTT 转换器；`scripts/logistics-mqtt-transformer.mjs` 按 `profiles/devices/maps` 配置把现场 PLC 原始报文转换为编辑器规范 Topic，默认 dry-run 可把 `DDJ2 front_command=5 + front_containerCode=BOX001 + to_z/to_x/to_y=1/9/3` 转换为 `cargo_action=drop`、`cargo=BOX001`、`target=1-9-3`，并保留 movement/distance 动画字段。已执行 `node --check scripts\logistics-mqtt-transformer.mjs` 和 `npm run demo:logistics-transformer:dry-run`。
- 2026-06-26：按 Stacker 最新点位截图调整 DDJ2 动画入参基准；`scripts/stacker-ddj2-plc-message.mjs`、`scripts/stacker-mqtt-demo-bridge.mjs` 和 `scripts/stacker-scene-mqtt-sequence.mjs` 默认现场包更新为 `front_signalBits=512`、`rpm_x=10`、`distance_x=52.1954`、`distance_y=0.3563`，`signalBits/front_signalBits/back_signalBits` 继续按 Integer 遥测字段保留，不拆 bit 驱动动画。已执行 `node --check scripts\stacker-ddj2-plc-message.mjs`、`node --check scripts\stacker-mqtt-demo-bridge.mjs`、`node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-ddj2-plc:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`npx tsc -b`、`git diff --check` 和 `npm run build`。
- 2026-06-25：按 DDJ2 Stacker 实际接收报文对齐动画数据；`scripts/stacker-ddj2-plc-message.mjs`、`scripts/stacker-mqtt-demo-bridge.mjs` 和 `scripts/stacker-scene-mqtt-sequence.mjs` 默认输出 `deviceCode/front_command/mode/back_command/front_x/front_y/signalBits/front_signalBits/back_signalBits/movement_*/distance_*/rpm_*/workingHours_*` 等完整现场点位，基准值更新为 `front_x=43`、`front_y=1`、`distance_x=52.2018`、`distance_y=0.2357`；场景流程的 `--plc` 不再把 DDJ2 动作转换成旧 `action/front_action/front_forkAction` 位域，动画目标字段作为扩展点位附加在同一 `data` 数组；运行时补齐 DDJ2 状态字段识别并保持这些字段只作遥测保留。已执行 `node --check scripts\stacker-ddj2-plc-message.mjs`、`node --check scripts\stacker-mqtt-demo-bridge.mjs`、`node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-ddj2-plc:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`npx tsc -b`、`git diff --check` 和 `npm run build`。
- 2026-06-25：按最新截图更正光电信号字段为 Integer 枚举；`scripts/stacker-scene-mqtt-sequence.mjs` 改为发送 `signalBits=0` 表示前端有货、`signalBits=3` 表示后端有货，不再使用旧位表达方式；运行时改为精确匹配枚举值，旧 `move` 来货信号兼容逻辑保留。已执行旧语义关键词检索、`node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`git diff --check` 和 `npm run build`。
- 2026-06-25：按链条机最新 MQTT 文档调整动画数据；`scripts/stacker-scene-mqtt-sequence.mjs` 的 `1004` 链条机报文收敛为 `deviceCode/mode/task/movement_x/movement_y/signalBits/containerCode/workingHours_x/workingHours_y/normal/errorCode/message/layer/rotation/container_quantity/folding/flip/fork/result/result2` 字段集合，时间戳输出为东八区 `+08:00`；运行时对链条机目标撤销由旧 `action` 位域推导的 `movement_x`，保证抱束业务状态不驱动输送方向，显式 `movement_x` 仍负责箭头和货箱推进。已执行 `node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`git diff --check` 和 `npm run build`。
- 2026-06-24：把 test2 主场景统一数据源从本地 demo WebSocket 切换为真实 MQTT over WebSocket：`ws://192.168.60.154:8083/mqtt`，Topic 为 `dt/factory/logistics/+/+/twinspawn` 和 `dt/factory/logistics/+/+/twindatadriven/joint`；已验证 TCP 端口可达、WebSocket `/mqtt` 子协议协商成功、MQTT `CONNACK=0` 和订阅 `SUBACK`。`joint` 已收到现场 conveyor 点位包，结构合理；当次指定监听 `1004` 和 `DDJ2` 未收到现场 `PUBLISH`。
- 2026-06-24：修复链条机开启动画后 Rail 与主体分离的问题；复核 test2 项目 `1004` 链条机静态场景模型装配正确，`1004` 的前端端点为世界坐标约 `(15.542, 0, 0.205)`，后端约 `(11.616, 0, 0.205)`，`movement_x=1` 按 `chainFrontEndpointRatio -> chainRearEndpointRatio` 沿世界 X 负方向输送。运行时补齐 Stacker `fork_target=Task202` 对运行态生成货箱的直接索引，避免临时 cube 因不进入普通场景树缓存而无法作为伸叉目标；链条机 motion 初始化会把旧模型包 `Rail_01_M001/Rail_02_M001` 的 `movement_x` 声明转换为货箱输送专用组，不再写 Rail 节点位置、旋转或 pivot，避免预览态破坏主体装配。已执行 `node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`npx tsc -b`、`git diff --check` 和 `npm run build`。
- 2026-06-24：链条机运行态新增输送面光晕方向箭头；`movement_x=1/2` 时箭头沿 `chainFrontEndpointRatio -> chainRearEndpointRatio` 或反向循环移动，`movement_x=0` 或断流时隐藏，Rail 链条模型继续保持静态装配不动；临时箭头节点和材质均标记为 `doNotSerialize` 并在退出预览时释放。
- 2026-06-24：修正 test2 链条机 `1004` 货箱半程停止的问题；示例脚本在 `movement_x=1` 输送期间每 2 秒补发链条机完整 PLC 点位包，持续刷新动作帧，直到按 `chainLength=3.926` 和前后端比例走到用户图示后端后再发送 `movement_x=0` 与后端有货信号。
- 2026-06-23：修正 `1004` 前端有货步骤未生成运行态货箱的问题；输送线光电信号现在会先按辊道机/链条机类型尝试生成或绑定运行态 cube，再判断是否具备可推进 motion 组，避免旧链条机 motion 识别失败时提前跳过生成；前端有货状态也改为货箱确实存在或创建成功后才记录，创建失败时同一高电平可继续重试；已执行 `npx tsc -b`、`node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`git diff --check` 和 `npm run build`。
- 2026-06-23：按输送线辊道机、链条机新 MQTT 点位更新动画示例；`scripts/stacker-scene-mqtt-sequence.mjs` 的 `1004` 报文改为 `{data:[{e,p,v}],ts}` 完整字段集合，后续已按最新截图更正为使用 `signalBits=0/3` 表示前后端光电，默认 `task=202` 且 `containerCode=""` 时运行态货箱编号为 `Task202`；运行时新增 `containerCode` 货箱编号候选，并保证新协议 `action` 抱机动作不会覆盖显式 `movement_x`；已执行 `node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`git diff --check` 和 `npm run build`。
- 2026-06-23：修正运行态货箱未沿链条机轨迹移动的问题；链条机生成的 cube 会记录并沿 `chainFrontEndpointRatio -> chainRearEndpointRatio` 端点世界线段推进，不再按根节点本地轴投影移动，旧链条机模型包缺少可解析 motion 时也会补默认 `movement_x` 货箱输送组；已执行 `npx tsc -b`、`node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、`git diff --check` 和 `npm run build`。
- 2026-06-23：完成 test2 `1004 -> DDJ2 -> 1-1-1` MQTT 运行态货箱流程；`1004` 前端有货信号会生成预览态 cube，脚本不再发布货箱位姿帧，DDJ2 继续用 `travel_target/fork_target/drop_target` 按模型目标取放货；已执行 `node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc`、dry-run 货箱位姿字段过滤检查、`git diff --check` 和 `npm run build`。
- 2026-06-23：修正链条机 `movement_x` 运行态动画，旧模型包声明为 `translate / z` 时会在预览态兼容为货箱输送动作组，货箱按链条机端点参数和 0.3m/s 推进；已执行 `node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc` 和 `npm run build`。
- 2026-06-22：完成 test2 `1005 -> DDJ2 -> 1-1-1` MQTT 模型目标定位动画；脚本从 test2 主场景读取 `1005`、`DDJ2`、`Box01`、`1-1-1` 模型位置和链条机参数，使用 `travel_target/fork_target/drop_target` 推导取放货目标，不再依赖距离定位字段；已执行 `npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc` 和 `npm run build`，构建通过，Vite 仅保留既有大 chunk 警告。
- 2026-06-22：RGV 模型包新增 `vehicleLength`、`vehicleWidth`、`vehicleHeight` 米制动态参数，并修复保存/刷新后参数化根缩放回弹；`meta.json` 会把三项显示到右侧属性面板，`rgv.model.ts` 运行脚本按默认米制尺寸基线映射长 Z、宽 X、高 Y 并实时缩放模型根节点；已执行 `npm run build`，构建通过。
...
- 2026-06-22：修复 RGV 修改长宽高参数后保存/刷新或实时应用时可能还原为未改参数外观的问题；模型包运行器在普通复制、阵列、销毁前仍会停止并清零根节点参数化缩放，但场景序列化保存时会保留 `metadata.editor.modelPackageRuntime.parametricRootScaling`；RGV 长宽高运行脚本改为按默认米制尺寸作为稳定比例基线，不再用当前世界包围盒作为下一次参数计算分母，避免修改完成后被恢复逻辑抵消回初始化外观。
...
- 2026-06-22：修正 Stacker 货叉参数化运行态语义；模型包实例参数 `forkLength` 现在作为动画伸叉的最大总长度，不再直接缩放编辑态静态货叉模型，并兼容 `fork_length`、中文“货叉长度”、中文“货叉总长度”和脚本字段默认值；运行时按取货方向生成左右双向行程，按 `front_forkLocation/back_forkLocation` 的 bit6/bit5 深位换算完整行程、bit4/bit3/bit1/bit0 浅位或换速位换算半段行程、bit2 归零，并把连续距离字段夹紧到当前实例的 `[-forkLength,+forkLength]`，且标准位置位优先于非标准连续距离字段；同帧前/后叉位置位会优先采用非原位活动位置，避免前叉原位遮住后叉深/浅位；Stacker 目标匹配会同时接受现场 `DDJ2` 和旧默认 `Stacker01`，避免新现场报文因模型资产编号未手动改成 `DDJ2` 而无法驱动；调整后的 Stacker 模型包默认货叉动画行程同步为 `0.941m`。
- 2026-06-22：按当前 DDJ2 MQTT 新报文重新接入 Stacker 动画；`{data:[{e,p,v}],ts}` 外层时间会透传到归一化帧，新字段 `movement_x/movement_y/front_movement_z/back_movement_z` 直接驱动行走、升降和伸叉，`distance_x/distance_y/front_distance_z/back_distance_z` 分别作为行走、载货台高度和货叉伸缩的绝对校准值，并优先于同帧动作积分；前后叉同组动作会优先采用非 0 动作，避免前叉停止值遮住后叉动作；`scripts/stacker-ddj2-plc-message.mjs` 已切换为截图字段集合并默认 `deviceCode=1`。旧 V5.2 的 `action/front_action/back_action/front_forkAction/back_forkAction/distancex/front_distanceY/back_distanceY/front_forkLocation/back_forkLocation` 继续兼容。
- 2026-06-22：按 test2 现场流程扩展 `scripts/stacker-scene-mqtt-sequence.mjs`；默认设备改为链条机 `1005`、堆垛机 `DDJ2`、货箱 `Box01` 和定位线框 `1-1-1`，报文现在覆盖前端有货、货箱绑定、链条输送、后端有货、DDJ2 伸叉取货、缩回载货台、移动到货位、定点放货和信号复位；test2 项目场景已新增资产编号 `1-1-1` 的定位线框，原场景已备份到 `scenes/.backups/`。
- 2026-06-23：链条机模型包新增 `chainFrontEndpointRatio` / `chainRearEndpointRatio` 前后端比例参数，默认沿局部 `-Z` 到 `+Z` 输送段取后端 `0`、前端 `1`；运行时 `target_anchor=front/rear` 命中链条机时优先读取该参数，test2 的 `scripts/stacker-scene-mqtt-sequence.mjs` 也用同一参数生成货箱前后端和输送插值，不再依赖手工距离定位。已执行 `node --check scripts\stacker-scene-mqtt-sequence.mjs`、`npm run demo:stacker-scene:dry-run`、`npm run demo:stacker-scene:dry-run -- --plc` 和 `npm run build`。
- 2026-06-23：修正链条机动画语义；`movement_x=1/2/0` 不再让 `Rail_01_M001/Rail_02_M001` 平移，后续已在 2026-06-24 收敛为只推进运行态货箱、不改写 Rail 节点变换，旧场景无需重新导入模型包；货箱绑定按链条机前后端点轨迹推进，保留 0.3m/s 输送语义。
- 2026-06-23：将 test2 场景流程切换为链条机 `1004` 前端有货生成运行态 cube；`scripts/stacker-scene-mqtt-sequence.mjs` 默认按 `task=202` 生成运行态货箱 `Task202`，不再要求场景预置 `Box01`，也不再发布货箱位姿帧，DDJ2 通过运行态货箱编号完成取货并放入 `1-1-1`。
- 2026-06-23：参考 ForeSim 发布场景调整编辑态和预览态鼠标视角操作；左键拖拽现在直接环绕当前 pivot，左键单击仍保留对象选择，命中 Gizmo 时不会抢占移动/旋转/缩放轴；右键拖拽改为平移视角，右键单击继续打开编辑态对象菜单，预览态仍抑制编辑菜单；滚轮和 `Alt + 右键` 保留沿视线缩放/前进后退的可穿模语义。
...
- 2026-06-13：工作网格升级为同步呼吸闪烁和专用 GlowLayer 光晕；光晕只 include 网格闪光主线、坐标轴和定位线，仍保持 helper 不参与拾取、层级树和场景序列化。
- 2026-06-21：左侧模型树拖拽归组支持当前多选集合；拖拽已选节点时会把选区内未锁定的顶层模型或 group 一起移入目标 group，拖到树空白根区域时会一起移回根级，并继续拦截拖入自身、后代或锁定 group 的非法层级。
- 2026-06-21：工具栏新增工作网格显示/隐藏开关和网格呼吸效果开关；工作网格隐藏时仍保留透明拖放平面用于资源落点，关闭呼吸效果时只保留基础网格线并关闭闪光层与 GlowLayer，画质、自动降载和大场景策略不再控制呼吸效果。
- 2026-06-21：补齐大场景流畅度优化第一阶段；Electron 启动增加 `enable-zero-copy`、Windows `use-angle=d3d11` 和窗口 `backgroundThrottling=false`，开发态 DevTools 改为 `ELECTRON_OPEN_DEVTOOLS=1` 显式开启；Babylon 新增“无损、自动、均衡、流畅”四档画质，默认无损高清不自动降画质，只有自动档会在低 FPS 时临时提高 `hardwareScalingLevel`；运行态新增场景统计、POI 节点、可绑定业务根节点和数据驱动目标缓存，大场景或低 FPS 时暂停指针移动拾取，减少 CPU 全量扫描和鼠标移动拾取开销；状态栏新增 WebGL 版本、最大纹理尺寸、软件渲染识别和自动降压提示。
...
- 2026-06-08：修复场景默认画质被旧性能开关覆盖成低 DPI 的问题；现在默认按 4K 目标像素量计算 `hardwareScalingLevel`，窗口尺寸变化或 WebGL 上下文恢复后都会重新应用高清策略，只有用户显式选择降负载画质才会降采样。
