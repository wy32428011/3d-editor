...
{
  "data": [
    {"e":"RollerConveyor01","p":"deviceCode","v":"RollerConveyor01"},
    {"e":"RollerConveyor01","p":"mode","v":2},
    {"e":"RollerConveyor01","p":"action","v":0},
    ...
    {"e":"RollerConveyor01","p":"normal","v":true},
    {"e":"RollerConveyor01","p":"errorCode","v":0},
    {"e":"RollerConveyor01","p":"message","v":"正常"},
    {"e":"RollerConveyor01","p":"layer","v":0},
    {"e":"RollerConveyor01","p":"rotation","v":0},
    {"e":"RollerConveyor01","p":"container_quantity","v":1},
    {"e":"RollerConveyor01","p":"folding","v":0},
    {"e":"RollerConveyor01","p":"flip","v":0},
    {"e":"RollerConveyor01","p":"fork","v":0},
    {"e":"RollerConveyor01","p":"result","v":0},
    {"e":"RollerConveyor01","p":"result2","v":0}
  ],
  "ts": "2026-06-23T13:59:25.969896635+08:00"
}
```
...
货箱被 `pickup` 吸附后，运行时会解除它与输送线的绑定，并在每帧把货箱姿态缓存同步到货叉吸附点；如果该货箱此前还有自身位姿流，吸附期间也会暂停消费。这样缩叉、载货台移动和整机移动时，货箱以 Stacker 货叉锚点为准持续跟随，不会被吸附前最后一帧目标位姿拉回输送线；`drop` 后再恢复可用的货箱自身位姿驱动。
...
前置条件：场景中需要有资产编号为 `1004` 的链条机、资产编号为 `DDJ2` 的 Stacker，以及资产编号为 `1-1-1` 的定位线框；不需要预置货箱 cube。脚本不会发布 `material/{boxId}/twinspawn` 货箱位姿帧，货箱生成、输送、取货和放货都由模型前端/后端锚点、运行态绑定关系和 Stacker 目标字段驱动。如果现场模型参数变化，先在编辑器中保存 test2 场景，脚本下次启动会重新读取主场景，并按链条机 `chainFrontEndpointRatio` / `chainRearEndpointRatio` 重新估算输送等待时间。可通过 `STACKER_SCENE_PROJECT_PATH` 覆盖项目路径，或用 `STACKER_SCENE_FILE_PATH` 指定单个 `.scene.json`；常用设备编号可覆盖 `STACKER_SCENE_STACKER_ID`、`STACKER_SCENE_CONVEYOR_ID`、`STACKER_SCENE_CONVEYOR_TASK`、`STACKER_SCENE_CONVEYOR_CONTAINER_CODE`、`STACKER_SCENE_BOX_ID`、`STACKER_SCENE_LOCATOR_ID`。本流程不会发送 `distance_x/distance_y/front_distance_z/back_distance_z` 来定位，但运行时仍兼容这些历史距离字段。

### 9.5 test2 真实 MQTT 现场接入

`C:\Users\WY\Documents\test2\test2` 当前主场景已配置为直接连接真实 MQTT over WebSocket：

- Broker WS 地址：`ws://192.168.60.154:8083/mqtt`
- 订阅 Topic：`dt/factory/logistics/+/+/twinspawn`、`dt/factory/logistics/+/+/twindatadriven/joint`
- 数据源类型：`MQTT`
- 设备字段：`e`
- 匹配字段：`assetCode`
- 插值：`200ms`

打开 test2 项目后直接进入预览即可连接该 broker。`twinspawn` 会驱动整机或物料根位姿，`twindatadriven/joint` 会驱动链条机、辊道机和 Stacker 内部动作；如果 payload 内缺少 `e`，规范 Topic 中的 `{deviceId}` 会兜底作为设备号匹配场景资产编号。当前按现场已存在 Topic 只订阅 `joint`，不订阅更宽的 `twindatadriven/#`，避免把 payload/status/alarm 一并接入动画链路。现场需要持续发送 `movement_x`、`front_movement_z` 等动作点位，模型才会出现内部机构动画。

2026-06-24 已完成低层连通性验证：`192.168.60.154:8083` TCP 可达，`/mqtt` WebSocket 握手成功且子协议为 `mqtt`，MQTT `CONNACK=0`，订阅上述 Topic 后收到 `SUBACK`。`dt/factory/logistics/+/+/twindatadriven/joint` 已收到现场 conveyor 设备的 `{data:[{e,p,v}],ts}` 完整点位包，结构与运行时兼容；当次 15 秒指定监听 `dt/factory/logistics/conveyor/1004/twindatadriven/joint`、`dt/factory/logistics/stacker/DDJ2/twindatadriven/joint` 和 `dt/factory/logistics/+/DDJ2/twindatadriven/joint` 未收到 `PUBLISH`。如果界面显示“已订阅”但 test2 模型不动，应优先核对现场当前是否正在向 `1004` 或 `DDJ2` 对应 Topic 推送 JSON 数据。
