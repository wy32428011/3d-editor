...
/**
 * 声明 RGV 的数据驱动设备绑定语义，编辑器导入时会静态解析该对象。
 */
...
// 此文件按模型参数化说明生成，用于 异形环穿车 的静态参数配置。
// 第一版只暴露静态参数，不包含旧动画、外部数据驱动或状态高亮运行逻辑。


/**
 * 管理 异形环穿车 在 Babylon.js Editor Inspector 中展示的静态参数。
 */
export class ParametricModelParamsComponent {
	@visibleAsString("模型标识")
	public modelKey: string = "rgv";

	@visibleAsString("设备类型")
	public deviceType: string = "RGV";

	@visibleAsString("设备名称")
	public deviceName: string = "异形环穿车";

	@visibleAsString("参数说明")
	public description: string = "支持 RGV 长宽高、路径点、路线类型、开放闭合、轨道显示、轨道宽度和单双工模式参数化。";

	@visibleAsString("路径点 JSON")
	public pathPoints: string = "[]";

	@visibleAsString("路径类型")
	public pathType: string = "straight";

	@visibleAsString("线路模式")
	public lineMode: string = "open";

	@visibleAsBoolean("闭合路线")
	public routeClosed: boolean = false;

	@visibleAsBoolean("显示轨道")
	public showTrack: boolean = true;

	@visibleAsNumber("RGV长度", { min: 0.1, max: 100, step: 0.1 })
	public vehicleLength: number = 10.3118;

	@visibleAsNumber("RGV宽度", { min: 0.1, max: 100, step: 0.1 })
	public vehicleWidth: number = 1.0;

	@visibleAsNumber("RGV高度", { min: 0.1, max: 20, step: 0.1 })
	public vehicleHeight: number = 0.7938;

	@visibleAsNumber("轨道宽度", { step: 0.01 })
	public trackWidth: number = 0.2;

	@visibleAsString("工作模式")
	public workMode: string = "single";
...
// 此文件按模型参数化说明生成，用于 异形环穿车 的静态参数化运行。
...
const DEFAULT_VALUES: ValueMap = {
	...
	"deviceType": "RGV",
	"deviceName": "异形环穿车",
	"description": "支持 RGV 长宽高、路径点、路线类型、开放闭合、轨道显示、轨道宽度和单双工模式参数化。",
	"pathPoints": "[]",
	"pathType": "straight",
	"lineMode": "open",
	"routeClosed": false,
	"showTrack": true,
	"vehicleLength": 10.3118,
	"vehicleWidth": 1.0,
	"vehicleHeight": 0.7938,
	"trackWidth": 0.2,
	"workMode": "single"
};

/**
 * 根据 Inspector 参数对 异形环穿车 执行静态参数化调整。
 */
export class ParametricModelRuntimeComponent {
	...
	private applyDimensionScale(values: ValueMap): void {
		const xScale = this.combineRatios(values, ["width", "lineWidth", "chainWidth", "bodyWidth", "vehicleWidth", "radius"], "x");
		const yScale = this.combineRatios(values, ["height", "lineHeight", "chainHeight", "bodyHeight", "vehicleHeight", "frontSupportHeight", "rearSupportHeight"], "y");
		const zScale = this.combineRatios(values, ["length", "lineLength", "chainLength", "bodyLength", "vehicleLength"], "z");
	...
	/**
	 * 判断是否使用参数默认值作为尺寸基线，避免根节点运行态缩放污染下一次比例计算。
	 */
	private shouldUseDefaultDimensionBaseline(key: string): boolean {
		return key === "vehicleLength" || key === "vehicleWidth" || key === "vehicleHeight";
	}
	...
	private ratioForNode(target: any, values: ValueMap, key: string, axis: "x" | "y" | "z"): number {
		const defaultMeters = this.readPositiveNumber(DEFAULT_VALUES, key, 1);
		const measuredMeters = this.getNodeWorldAxisSize(target, axis);
		const fallbackMeters = this.shouldUseDefaultDimensionBaseline(key)
			? defaultMeters
			: measuredMeters > 0 ? measuredMeters : defaultMeters;
