...
export const dataDriven = {
	...
	motion: {
		chain: {
			...
			speed: 0.3,
			nodes: ["Rail_01_M001", "Rail_02_M001"],
			fallbackPattern: "chain|链|rail|Rail"
		}
	}
} as const;

// 此文件按模型参数化说明生成，用于 链条输送机 的静态参数配置。
// 第一版只暴露静态参数，不包含旧动画、外部数据驱动或状态高亮运行逻辑。


/**
 * 管理 链条输送机 在 Babylon.js Editor Inspector 中展示的静态参数。
 */
export class ParametricModelParamsComponent {
	@visibleAsString("模型标识")
	public modelKey: string = "chain-conveyor";

	@visibleAsString("设备类型")
	public deviceType: string = "输送";

	@visibleAsString("设备名称")
	public deviceName: string = "链条输送机";

	@visibleAsString("参数说明")
	public description: string = "支持链条机长、宽、位置和前后支架显示隐藏参数化。";

	@visibleAsNumber("链条机长度", { step: 0.1 })
	public chainLength: number = 1.826;

	@visibleAsNumber("链条机宽度", { step: 0.1 })
	public chainWidth: number = 1.271;

	@visibleAsNumber("链条机位置", { step: 0.1 })
...
// 此文件按模型参数化说明生成，用于 链条输送机 的静态参数化运行。
...
const DEFAULT_VALUES: ValueMap = {
	...
	"deviceType": "输送",
	"deviceName": "链条输送机",
	"description": "支持链条机长、宽、位置和前后支架显示隐藏参数化。",
	"chainLength": 1.826,
	"chainWidth": 1.271,
	"chainPosition": 0,
...
/**
 * 根据 Inspector 参数对 链条输送机 执行静态参数化调整。
 */
export class ParametricModelRuntimeComponent {
	...
	private applyDimensionScale(values: ValueMap): void {
		const xScale = this.combineRatios(values, ["width", "lineWidth", "bodyWidth"], "x");
		const yScale = this.combineRatios(values, ["height", "lineHeight", "chainHeight", "bodyHeight", "frontSupportHeight", "rearSupportHeight"], "y");
		const zScale = this.combineRatios(values, ["length", "lineLength", "bodyLength", "vehicleLength", "radius"], "z");
