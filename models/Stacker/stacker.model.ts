...
export class ParametricModelRuntimeComponent {
	...
	private applyStackerBodyHeight(values: ValueMap): void {
		...
		const mastBounds = this.getWorldYBounds(mastMeshes);
		...
		this.stretchMeshesFromWorldYMin(mastMeshes, clampedScale);
		this.offsetNodesByWorldDelta(this.findStackerTopLiftNodes(), new Vector3(0, extension, 0));
	}
	...
	private stretchMeshesByWorldZ(meshes: any[], lengthScale: number): void {
		...
		const bounds = this.getWorldZBounds(meshes);
		if (!bounds) { return; }
		...
		meshes.forEach((mesh) => this.stretchMeshByWorldZ(mesh, sourceMiddleStart, sourceMiddleEnd, sourceMiddleLength, middleScale, extension));
	}
	 ...
	 */
	private stretchMeshByWorldZ(mesh: any, sourceMiddleStart: number, sourceMiddleEnd: number, sourceMiddleLength: number, middleScale: number, extension: number): void {
		...
		for (let index = 0; index < nextPositions.length; index += 3) {
			...
			const nextWorldZ = this.mapAnchoredWorldZ(world.z, sourceMiddleStart, sourceMiddleEnd, sourceMiddleLength, middleScale, extension);
			if (Math.abs(nextWorldZ - world.z) < 0.0001) { continue; }
			const nextLocal = Vector3.TransformCoordinates(new Vector3(world.x, world.y, nextWorldZ), inverseWorldMatrix);
	 ...
	 */
	private mapAnchoredWorldZ(worldZ: number, sourceMiddleStart: number, sourceMiddleEnd: number, sourceMiddleLength: number, middleScale: number, extension: number): number {
		if (worldZ <= sourceMiddleStart) { return worldZ; }
		if (worldZ >= sourceMiddleEnd) { return worldZ + extension; }
		return sourceMiddleStart + (worldZ - sourceMiddleStart) / sourceMiddleLength * sourceMiddleLength * middleScale;
	}
	 ...
	 */
	private getWorldZBounds(meshes: any[]): { min: number; max: number } | null {
		...
		meshes.forEach((mesh) => {
			...
			for (let index = 0; index < positions.length; index += 3) {
				...
				min = Math.min(min, world.z);
				max = Math.max(max, world.z);
			}
		});
	...
	private scaleNodeByWorldAxes(target: any, worldXScale: number, worldYScale: number, worldZScale: number): void {
		const snapshot = this.rememberSnapshot(target);
		if (!target.scaling) { return; }
		const localScale = this.getLocalScaleForWorldAxes(snapshot, worldXScale, worldYScale, worldZScale);
	 ...
	 */
	private getLocalScaleForWorldAxes(snapshot: NodeSnapshot, worldXScale: number, worldYScale: number, worldZScale: number): Vector3 {
		const quaternion = this.getSnapshotQuaternion(snapshot);
		if (!quaternion) { return new Vector3(worldXScale, worldYScale, worldZScale); }
		return new Vector3(
			this.pickWorldScale(this.rotateLocalAxis(new Vector3(1, 0, 0), quaternion), worldXScale, worldYScale, worldZScale),
			this.pickWorldScale(this.rotateLocalAxis(new Vector3(0, 1, 0), quaternion), worldXScale, worldYScale, worldZScale),
			this.pickWorldScale(this.rotateLocalAxis(new Vector3(0, 0, 1), quaternion), worldXScale, worldYScale, worldZScale),
		);
	}

	/**
	 * 读取节点快照中的旋转四元数；没有四元数时兼容欧拉角。
	 */
	private getSnapshotQuaternion(snapshot: NodeSnapshot): { x: number; y: number; z: number; w: number } | null {
		if (snapshot.rotationQuaternion) { return snapshot.rotationQuaternion; }
		if (!snapshot.rotation) { return null; }
		return this.eulerToQuaternion(snapshot.rotation);
	}

	/**
	 * 将 Babylon 欧拉角转换成四元数，用于没有 rotationQuaternion 的普通节点。
	 */
	private eulerToQuaternion(rotation: Vector3): { x: number; y: number; z: number; w: number } {
	 ...
	 * 使用四元数把局部轴方向转换到世界坐标系。
	 */
	private rotateLocalAxis(axis: Vector3, quaternion: { x: number; y: number; z: number; w: number }): Vector3 {
		const rawX = Number(quaternion.x);
		const rawY = Number(quaternion.y);
		const rawZ = Number(quaternion.z);
		const rawW = Number(quaternion.w);
		const qx = Number.isFinite(rawX) ? rawX : 0;
		const qy = Number.isFinite(rawY) ? rawY : 0;
		const qz = Number.isFinite(rawZ) ? rawZ : 0;
		const qw = Number.isFinite(rawW) ? rawW : 1;
		const tx = 2 * (qy * axis.z - qz * axis.y);
		const ty = 2 * (qz * axis.x - qx * axis.z);
		const tz = 2 * (qx * axis.y - qy * axis.x);
		return new Vector3(
			axis.x + qw * tx + qy * tz - qz * ty,
			axis.y + qw * ty + qz * tx - qx * tz,
			axis.z + qw * tz + qx * ty - qy * tx,
		);
	}
	 ...
	 * 按局部轴最接近的世界轴选择缩放倍率。
	 */
	private pickWorldScale(axis: Vector3, worldXScale: number, worldYScale: number, worldZScale: number): number {
		const x = Math.abs(axis.x);
		const y = Math.abs(axis.y);
		const z = Math.abs(axis.z);
	...
	private applyStackerPlatformHeight(values: ValueMap, platformNodes: any[]): void {
		...
		const platformBounds = this.getWorldYBounds(platformMeshes);
		if (!platformBounds) { return; }
		const profile = this.getPlatformHeightProfile(platformMeshes, platformBounds);
		if (!profile) { return; }
		...
		if (Math.abs(heightScale - 1) < 0.0001) { return; }
		this.stretchPlatformMeshesByHeight(platformMeshes, platformBounds, profile, heightScale);
	}

	/**
	 * 识别载货台高度语义边界：主体底部、底部保护区顶部和顶部保护区底部。
	 */
	private getPlatformHeightProfile(meshes: any[], bounds: { min: number; max: number }): { bodyBottomY: number; bottomProtectTopY: number; topProtectBottomY: number } | null {
		const sourceHeight = bounds.max - bounds.min;
		if (sourceHeight <= 0) { return null; }
		const levels = this.collectWorldYLevels(meshes);
	 ...
	 */
	private collectWorldYValues(meshes: any[]): number[] {
		const values: number[] = [];
		meshes.forEach((mesh) => {
			const positions = this.getBaseVertexPositions(mesh);
			const worldMatrix = mesh.computeWorldMatrix?.(true);
	 ...
	 */
	private collectWorldYLevels(meshes: any[]): number[] {
		const sortedValues = this.collectWorldYValues(meshes).sort((a, b) => a - b);
		const levels: number[] = [];
	 ...
	 */
	private stretchPlatformMeshesByHeight(meshes: any[], bounds: { min: number; max: number }, profile: { bodyBottomY: number; bottomProtectTopY: number; topProtectBottomY: number }, heightScale: number): void {
		...
		meshes.forEach((mesh) => this.stretchMeshBetweenWorldYBand(mesh, profile.bottomProtectTopY, profile.topProtectBottomY, middleScale, effectiveExtension));
	}

	/**
	 * 分段映射单个 mesh 的世界 Y 坐标：底部保护区不动，中间段拉伸，顶部保护区整体上移。
	 */
	private stretchMeshBetweenWorldYBand(mesh: any, bottomY: number, topY: number, middleScale: number, extension: number): void {
		const positions = this.getBaseVertexPositions(mesh);
		const worldMatrix = mesh.computeWorldMatrix?.(true);
		const inverseWorldMatrix = worldMatrix?.clone?.();
		if (!positions || !worldMatrix || !inverseWorldMatrix?.invert) { return; }
		...
		for (let index = 0; index < nextPositions.length; index += 3) {
			const local = new Vector3(positions[index], positions[index + 1], positions[index + 2]);
			const world = Vector3.TransformCoordinates(local, worldMatrix);
			const nextWorldY = this.mapPlatformWorldY(world.y, bottomY, topY, sourceMiddleHeight, middleScale, extension);
			if (Math.abs(nextWorldY - world.y) < 0.0001) { continue; }
			const nextLocal = Vector3.TransformCoordinates(new Vector3(world.x, nextWorldY, world.z), inverseWorldMatrix);
			nextPositions[index] = nextLocal.x;
			nextPositions[index + 1] = nextLocal.y;
			nextPositions[index + 2] = nextLocal.z;
		}
	 ...
	 */
	private mapPlatformWorldY(worldY: number, bottomY: number, topY: number, sourceMiddleHeight: number, middleScale: number, extension: number): number {
		if (worldY <= bottomY) { return worldY; }
		if (worldY >= topY) { return worldY + extension; }
		return bottomY + (worldY - bottomY) / sourceMiddleHeight * sourceMiddleHeight * middleScale;
	}

	/**
	 * 以世界 Y 最低点为锚点拉伸 mesh，底部保持原位，顶部向上或向下变化。
	 */
	private stretchMeshesFromWorldYMin(meshes: any[], heightScale: number): void {
		if (meshes.length === 0 || Math.abs(heightScale - 1) < 0.0001) { return; }
		const bounds = this.getWorldYBounds(meshes);
		if (!bounds) { return; }
		const sourceHeight = bounds.max - bounds.min;
		if (sourceHeight <= 0) { return; }
		const clampedScale = Math.max(1 / sourceHeight, heightScale);
		this.stretchMeshesAboveWorldYAnchor(meshes, bounds.min, clampedScale);
	}

	/**
	 * 以指定世界 Y 锚点向上拉伸 mesh，锚点以下顶点保持原位。
	 */
	private stretchMeshesAboveWorldYAnchor(meshes: any[], anchorY: number, heightScale: number): void {
		meshes.forEach((mesh) => this.stretchMeshAboveWorldYAnchor(mesh, anchorY, heightScale));
	}

	/**
	 * 拉伸单个 mesh 的世界 Y 坐标，保持锚点及锚点以下顶点不动。
	 */
	private stretchMeshAboveWorldYAnchor(mesh: any, anchorY: number, heightScale: number): void {
		const positions = this.getBaseVertexPositions(mesh);
		const worldMatrix = mesh.computeWorldMatrix?.(true);
		const inverseWorldMatrix = worldMatrix?.clone?.();
		if (!positions || !worldMatrix || !inverseWorldMatrix?.invert) { return; }
		...
		for (let index = 0; index < nextPositions.length; index += 3) {
			const local = new Vector3(positions[index], positions[index + 1], positions[index + 2]);
			const world = Vector3.TransformCoordinates(local, worldMatrix);
			if (world.y <= anchorY) { continue; }
			const nextWorldY = anchorY + (world.y - anchorY) * heightScale;
			if (Math.abs(nextWorldY - world.y) < 0.0001) { continue; }
			const nextLocal = Vector3.TransformCoordinates(new Vector3(world.x, nextWorldY, world.z), inverseWorldMatrix);
			nextPositions[index] = nextLocal.x;
			nextPositions[index + 1] = nextLocal.y;
	 ...
	 */
	private ratioForNodesByWorldAxis(nodes: any[], values: ValueMap, key: string, axis: "x" | "y" | "z"): number {
	 ...
	 */
	private ratioForMeshesByWorldAxis(meshes: any[], values: ValueMap, key: string, axis: "x" | "y" | "z"): number {
	 ...
	 */
	private getWorldAxisBounds(meshes: any[], axis: "x" | "y" | "z"): { min: number; max: number } | null {
		if (axis === "y") { return this.getWorldYBounds(meshes); }
		if (axis === "z") { return this.getWorldZBounds(meshes); }
		return this.getWorldXBounds(meshes);
	}

	/**
	 * 计算一组 mesh 的世界 X 包围范围。
	 */
	private getWorldXBounds(meshes: any[]): { min: number; max: number } | null {
		let min = Number.POSITIVE_INFINITY;
		let max = Number.NEGATIVE_INFINITY;
		meshes.forEach((mesh) => {
			const positions = this.getBaseVertexPositions(mesh);
