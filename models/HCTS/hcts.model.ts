...
/**
 * 声明换层提升机的数据驱动运动语义，编辑器导入时会静态解析该对象。
 */
export const dataDriven = {
	device: {
		devType: "lifter",
		defaultAssetCode: "HCTS01",
		deviceIdField: "e",
	...
	motion: {
		lift: {
			...
			speed: 0.3,
			nodes: ["对象011", "对象010", "对象029", "对象030", "对象031", "对象008", "对象007", "对象009"],
			limits: { min: 0, max: 2.5 }
		}
	}
} as const;

// 此文件按模型参数化说明生成，用于 换层提升机 的静态参数配置。
// 第一版只暴露静态参数，不包含旧动画、外部数据驱动或状态高亮运行逻辑。


/**
 * 管理 换层提升机 在 Babylon.js Editor Inspector 中展示的静态参数。
 */
export class ParametricModelParamsComponent {
	@visibleAsString("模型标识")
	public modelKey: string = "hcts";

	@visibleAsString("设备类型")
	public deviceType: string = "多穿库";

	@visibleAsString("设备名称")
	public deviceName: string = "换层提升机";

	@visibleAsString("参数说明")
	public description: string = "支持货仓适配、货架高度适配、前后切换和主体尺寸参数化。";

	@visibleAsBoolean("货仓自动适配穿梭车")
	public cargoBayAutoFitShuttle: boolean = true;

	@visibleAsBoolean("高度适配货架")
	public fitShelfHeight: boolean = true;

	@visibleAsString("切换方向")
	public switchDirection: string = "front";

	@visibleAsString("前后模式")
	public frontBackMode: string = "front";

	@visibleAsNumber("主体长度", { step: 0.1 })
	public bodyLength: number = 2.13;

	@visibleAsNumber("主体宽度", { step: 0.1 })
	public bodyWidth: number = 1.609;

	@visibleAsNumber("主体高度", { step: 0.1 })
	public bodyHeight: number = 5.395;
...
// 此文件按模型参数化说明生成，用于 换层提升机 的静态参数化运行。
...
const DEFAULT_VALUES: ValueMap = {
	...
	"deviceType": "多穿库",
	"deviceName": "换层提升机",
	"description": "支持货仓适配、货架高度适配、前后切换和主体尺寸参数化。",
	"cargoBayAutoFitShuttle": true,
	"fitShelfHeight": true,
...
/**
 * 根据 Inspector 参数对 换层提升机 执行静态参数化调整。
 */
