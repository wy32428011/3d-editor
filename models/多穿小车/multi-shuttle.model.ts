...
/**
 * 声明多穿小车的数据驱动运动语义，编辑器导入时会静态解析该对象。
 */
export const dataDriven = {
	device: {
		devType: "shuttle",
		defaultAssetCode: "Shuttle01",
		deviceIdField: "e",
...
// 此文件按模型参数化说明生成，用于 穿梭车 的静态参数配置。
// 第一版只暴露静态参数，不包含旧动画、外部数据驱动或状态高亮运行逻辑。


/**
 * 管理 穿梭车 在 Babylon.js Editor Inspector 中展示的静态参数。
 */
export class ParametricModelParamsComponent {
	@visibleAsString("模型标识")
	public modelKey: string = "multi-shuttle";

	@visibleAsString("设备类型")
	public deviceType: string = "多穿库";

	@visibleAsString("设备名称")
	public deviceName: string = "穿梭车";

	@visibleAsString("参数说明")
	public description: string = "支持车辆长度、货叉间距和数量参数化。";

	@visibleAsNumber("车辆长度", { step: 0.1 })
	public vehicleLength: number = 0.984;

	@visibleAsNumber("货叉间距", { step: 0.05 })
	public forkGap: number = 0.6;

	@visibleAsNumber("数量", { step: 1 })
...
const DEFAULT_VALUES: ValueMap = {
	"modelKey": "multi-shuttle",
	"deviceType": "多穿库",
	"deviceName": "穿梭车",
	"description": "支持车辆长度、货叉间距和数量参数化。",
	"vehicleLength": 0.984,
	"forkGap": 0.6,
	"count": 1
};
...
/**
 * 根据 Inspector 参数对 穿梭车 执行静态参数化调整。
 */
