...
/**
 * 声明弯道输送机的数据驱动运动语义，编辑器导入时会静态解析该对象。
 */
export const dataDriven = {
	device: {
		devType: "conveyor",
		defaultAssetCode: "WLTS01",
		deviceIdField: "e",
...
// 此文件按模型参数化说明生成，用于 弯道输送机 的静态参数配置。
// 第一版只暴露静态参数，不包含旧动画、外部数据驱动或状态高亮运行逻辑。


/**
 * 管理 弯道输送机 在 Babylon.js Editor Inspector 中展示的静态参数。
 */
export class ParametricModelParamsComponent {
	@visibleAsString("模型标识")
	public modelKey: string = "wlts";

	@visibleAsString("设备类型")
	public deviceType: string = "输送";

	@visibleAsString("设备名称")
	public deviceName: string = "弯道输送机";

	@visibleAsString("参数说明")
	public description: string = "支持半径、高度、角度、宽度、辊道密度和前后支架参数化。";

	@visibleAsNumber("半径", { step: 0.1 })
	public radius: number = 1.308;

	@visibleAsNumber("高度", { step: 0.1 })
	public height: number = 5.403;

	@visibleAsNumber("角度", { step: 1 })
	public angle: number = 90;

	@visibleAsNumber("宽度", { step: 0.1 })
	public width: number = 1.115;

	@visibleAsNumber("辊道密度", { step: 1 })
	public rollerDensity: number = 1;

	@visibleAsNumber("前支架高度", { step: 0.1 })
	public frontSupportHeight: number = 1;

	@visibleAsNumber("后支架高度", { step: 0.1 })
	public rearSupportHeight: number = 1;
...
// 此文件按模型参数化说明生成，用于 弯道输送机 的静态参数化运行。
...
const DEFAULT_VALUES: ValueMap = {
	...
	"deviceType": "输送",
	"deviceName": "弯道输送机",
	"description": "支持半径、高度、角度、宽度、辊道密度和前后支架参数化。",
	"radius": 1.308,
	"height": 5.403,
	"angle": 90,
	"width": 1.115,
	"rollerDensity": 1,
	"frontSupportHeight": 1,
	"rearSupportHeight": 1,
...
/**
 * 根据 Inspector 参数对 弯道输送机 执行静态参数化调整。
 */
