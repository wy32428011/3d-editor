...
/**
 * 声明辊道机的数据驱动运动语义，编辑器导入时会静态解析该对象。
 */
export const dataDriven = {
	device: {
		devType: "conveyor",
		defaultAssetCode: "RollerConveyor01",
		deviceIdField: "e",
	...
	motion: {
		roller: {
			...
			speed: 360,
			nodes: ["GT1", "GT2", "GT3", "GT4", "GT5", "GT6", "GT7", "GT8", "GT9", "GT10"],
			fallbackPattern: "GT|roller|辊|滚"
		}
	}
} as const;

// 此文件按模型参数化说明生成，用于 输送机 的静态参数配置。
// 第一版只暴露静态参数，不包含旧动画、外部数据驱动或状态高亮运行逻辑。


/**
 * 管理 输送机 在 Babylon.js Editor Inspector 中展示的静态参数。
 */
export class ParametricModelParamsComponent {
	@visibleAsString("模型标识")
	public modelKey: string = "roller-conveyor";

	@visibleAsString("设备类型")
	public deviceType: string = "输送";

	@visibleAsString("设备名称")
	public deviceName: string = "输送机";

	@visibleAsString("参数说明")
	public description: string = "支持输送线长宽高、辊筒宽度、辊筒数量和支架显示隐藏参数化。";

	@visibleAsNumber("长度", { step: 0.1 })
	public length: number = 1;

	@visibleAsNumber("宽度", { step: 0.1 })
	public width: number = 0.767;

	@visibleAsNumber("高度", { step: 0.1 })
	public height: number = 0.49;

	@visibleAsNumber("辊筒宽度", { step: 0.1 })
	public rollerWidth: number = 0.667;

	@visibleAsNumber("辊筒数量", { step: 1 })
	public rollerDensity: number = 10;
...
const DEFAULT_VALUES: ValueMap = {
	"modelKey": "roller-conveyor",
	"deviceType": "输送",
	"deviceName": "输送机",
	"description": "支持输送线长宽高、辊筒宽度、辊筒数量和支架显示隐藏参数化。",
	"length": 1,
	"width": 0.767,
	"height": 0.49,
	"rollerWidth": 0.667,
...
/**
 * 根据 Inspector 参数对 输送机 执行静态参数化调整。
 */
export class ParametricModelRuntimeComponent {
	...
	/**
	 * 根据辊筒数量重排可识别辊筒，输入值表示最终可见辊筒总数。
	 */
	private applyRollerDensity(values: ValueMap): void {
		const rollers = this.getRollerTemplates();
		...
		const targetCount = this.clamp(Math.round(this.readNumber(values, "rollerDensity", rollers.length)), 1, 80);
		const targetCenters = this.createRollerTargetCenters(rollers, targetCount);
		rollers.forEach((roller, index) => {
			const visible = index < Math.min(targetCount, rollers.length);
			if (typeof roller.setEnabled === "function") { roller.setEnabled(visible); }
			if (visible) { this.moveNodeAxisCenterTo(roller, "x", targetCenters[index]); }
		});
		for (let index = rollers.length; index < targetCount; index += 1) {
			const source = rollers[index % rollers.length];
			const clone = this.cloneSingleNode(source, Vector3.Zero(), "roller", index + 1);
			if (clone) { this.moveNodeAxisCenterTo(clone, "x", targetCenters[index]); }
		}
	}

	/**
	 * 获取原始辊筒模板并按输送线长度方向排序，避免运行时生成节点参与下一轮布局。
	 */
	private getRollerTemplates(): any[] {
		return this.findNodes(/roller|辊|滚|gt\d*/i)
			.filter((node) => !node.metadata?.generatedByParametricRuntime)
			.sort((left, right) => this.compareNodesByAxisCenter(left, right, "x"));
	}

	/**
	 * 在原始辊筒覆盖的长度范围内均匀生成目标中心点。
	 */
	private createRollerTargetCenters(rollers: any[], targetCount: number): number[] {
		const centers = rollers
			.map((roller) => this.getNodeAxisCenter(roller, "x"))
			.filter((center): center is number => Number.isFinite(center));
		const fallbackSpan = Math.max(0, (targetCount - 1) * this.readPositiveNumber(DEFAULT_VALUES, "rollerWidth", 1));
		const start = centers.length > 0 ? Math.min(...centers) : -fallbackSpan / 2;
		const end = centers.length > 0 ? Math.max(...centers) : fallbackSpan / 2;
		if (targetCount <= 1) { return [(start + end) / 2]; }
		const span = end - start;
		if (Math.abs(span) < 0.000001) {
			const spacing = this.readPositiveNumber(DEFAULT_VALUES, "rollerWidth", 1);
	...
	/**
	 * 按世界包围盒中心排序节点，保证辊筒重排与可见位置一致。
	 */
	private compareNodesByAxisCenter(left: any, right: any, axis: "x" | "y" | "z"): number {
		const leftCenter = this.getNodeAxisCenter(left, axis);
		const rightCenter = this.getNodeAxisCenter(right, axis);
	...
	/**
	 * 读取节点包围盒在指定轴上的局部中心，根节点缩放后仍能得到稳定布局坐标。
	 */
	private getNodeAxisCenter(target: any, axis: "x" | "y" | "z"): number {
		const bounds = this.getNodeWorldBounds(target);
		if (!bounds) { return 0; }
		const worldCenter = (bounds.minimum[axis] + bounds.maximum[axis]) / 2;
		const rootPosition = Number(this.node.position?.[axis] ?? 0);
		const rootScale = Number(this.node.scaling?.[axis] ?? 1);
		return Math.abs(rootScale) > 0.000001 ? (worldCenter - rootPosition) / rootScale : worldCenter - rootPosition;
	}

	/**
	 * 平移节点，使其包围盒中心落到指定轴向坐标。
	 */
	private moveNodeAxisCenterTo(target: any, axis: "x" | "y" | "z", center: number): void {
		if (!target.position || !Number.isFinite(center)) { return; }
		const currentCenter = this.getNodeAxisCenter(target, axis);
		const delta = center - currentCenter;
		if (!Number.isFinite(delta) || Math.abs(delta) < 0.000001) { return; }
		const nextPosition = target.position.clone?.() ?? this.rememberSnapshot(target).position.clone();
