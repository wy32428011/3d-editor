...
/**
 * 声明有电机辊道的数据驱动运动语义，编辑器导入时会静态解析该对象。
 */
export const dataDriven = {
	device: {
		devType: "conveyor",
		defaultAssetCode: "MotorConveyor01",
		deviceIdField: "e",
	...
	motion: {
		roller: {
			...
			speed: 360,
			nodes: ["GD_1", "GD_2", "GD_3", "GD_4", "GD_5", "GD_6", "GD_7", "GD_8", "GD_9", "GD_027", "GD_028", "GD_029"],
			fallbackPattern: "GD_|roller|辊|滚"
		}
	}
} as const;

// 此文件按模型参数化说明生成，用于 有电机辊道 的静态参数配置。
// 第一版只暴露静态参数，不包含旧动画、外部数据驱动或状态高亮运行逻辑。


/**
 * 管理 有电机辊道 在 Babylon.js Editor Inspector 中展示的静态参数。
 */
export class ParametricModelParamsComponent {
	@visibleAsString("模型标识")
	public modelKey: string = "gd-motor-optimized";

	@visibleAsString("设备类型")
	public deviceType: string = "输送";

	@visibleAsString("设备名称")
	public deviceName: string = "有电机辊道";

	@visibleAsString("参数说明")
	public description: string = "支持有电机辊道长宽高、辊筒宽度、辊筒密度和支架显示隐藏参数化。";

	@visibleAsNumber("长度", { step: 0.1 })
	public length: number = 3.051;

	@visibleAsNumber("宽度", { step: 0.1 })
	public width: number = 1.186;

	@visibleAsNumber("高度", { step: 0.1 })
	public height: number = 1.06;

	@visibleAsNumber("辊筒宽度", { step: 0.1 })
	public rollerWidth: number = 3.051;
...
// 此文件按模型参数化说明生成，用于 有电机辊道 的静态参数化运行。
...
const DEFAULT_VALUES: ValueMap = {
	"modelKey": "gd-motor-optimized",
	"deviceType": "输送",
	"deviceName": "有电机辊道",
	"description": "支持有电机辊道长宽高、辊筒宽度、辊筒密度和支架显示隐藏参数化。",
	"length": 3.051,
	"width": 1.186,
	"height": 1.06,
	"rollerWidth": 3.051,
...
/**
 * 根据 Inspector 参数对 有电机辊道 执行静态参数化调整。
 */
export class ParametricModelRuntimeComponent {
	...
	private applyDimensionScale(values: ValueMap): void {
		const xScale = this.combineRatios(values, ["width", "lineWidth", "chainWidth", "bodyWidth"], "x");
		const yScale = this.combineRatios(values, ["height", "lineHeight", "chainHeight", "bodyHeight", "frontSupportHeight", "rearSupportHeight"], "y");
		const zScale = this.combineRatios(values, ["length", "lineLength", "chainLength", "bodyLength", "vehicleLength", "radius"], "z");
